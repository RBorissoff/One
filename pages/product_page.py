...
class ProductPage(BasePage):
    ...
    def should_be_promo_in_the_url(self):
        assert "promo" in self.browser.current_url, "There's no promo in the link"
    ...
    def added_book_and_chosen_should_be_equal(self): 
        ...
        added_book_msg = self.browser.find_element(*ProductPageLocators.ADDED_BOOK_IN_MESSAGE).text
        assert book_name == added_book_msg, "Added to cart book and chosen one are different"

    def prices_in_cart_and_chosen_book_should_be_equal(self):
        book_price = self.get_book_price()
        added_book_price_in_msg = self.browser.find_element(*ProductPageLocators.TOTAL_PRICE_IN_CART_MESSAGE).text
        assert book_price == added_book_price_in_msg, "Total sum in cart is not equal to added one"
